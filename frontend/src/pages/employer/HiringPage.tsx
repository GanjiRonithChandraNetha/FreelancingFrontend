...
const AgreedFreelancers: React.FC = () => {
  ...
  const fetchAgreedApplicants = async (): Promise<void> => {
    try {
      setLoading(true);
      const response = await axios.post<ApiResponse>(`${rootUrl}/v1/application/employer/job/read/agreed`, {
        jobId: jobId
      },
      {
        headers: {
  ...
  const appointFreelancer = async (applicationId: string): Promise<void> => {
    try {
      ...
      const response = await axios.post<ApiResponse>(`${rootUrl}/v1/application/employer/job/read/provied-project`, {
            jobId: jobId,
