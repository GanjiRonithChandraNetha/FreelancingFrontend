...
//   console.log(jobId);
  ...
  useEffect(() => {
    ...
    const fetchJobDetails = async () => {
      try {
        // console.log(jobId);
        const response = await axios.get(`${rootUrl}/v1/application/viewJob/${jobId}`,
            {
                headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${rootUrl}/v1/application/freelancer/application/create`, applicationData,
        {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("FreeToken")}`, // Example: Authorization header
              'Content-Type': 'application/json',     // Example: Content-Type
